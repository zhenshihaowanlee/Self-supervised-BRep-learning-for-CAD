...
class BRepNetExtractor:
    ...
    def extract_features_from_face(self, face):
        ...
        for feature in self.feature_schema["face_features"]:
            ...
            elif feature == "FaceNormalFeature":
                face_features.append(self.face_normal_feature(face))  # 新增
            else:
                raise ValueError(f"Unknown face feature: {feature}")
        return np.array(face_features)

    def face_normal_feature(self, face):
      surf = BRepAdaptor_Surface(face)
      u_min, u_max, v_min, v_max = surf.Bounds()
      u_mid = (u_min + u_max) / 2
      v_mid = (v_min + v_max) / 2
      normal = surf.Surface().DN(u_mid, v_mid, 1, 0)  # 计算法向量
      normal_magnitude = np.linalg.norm(normal)
      return normal / normal_magnitude if normal_magnitude > 0 else np.zeros(3)  
    ...
    def extract_features_from_edge(self, edge, faces):
        ...
        for feature in feature_list:
            if feature == "LineEdgeFeature":
                edge_features.append(self.line_edge_feature(edge))
            ...
            elif feature == "EdgeMidpointFeature":
                edge_features.append(self.edge_midpoint_feature(edge))  # 新增
            else:
                raise ValueError(f"Unknown edge feature: {feature}")
        return np.array(edge_features)
        
    def line_edge_feature(self, edge):
    ...
    def edge_midpoint_feature(self, edge):
        edge_curve = BRepAdaptor_Curve(edge)
        u_min, u_max = edge_curve.FirstParameter(), edge_curve.LastParameter()
        midpoint = edge_curve.Value((u_min + u_max) / 2)  # 中点计算
